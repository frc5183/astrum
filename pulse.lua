...
function Pulse:removeOnEvent(event, id)
  ...
  if (type(self.events[event] ~= "table")) then
    safety.ensureTable(self.events[event],
      "event " ..
...
function Pulse:require(event, id, ...)
  ...
  safety.ensureFunction(self.events[event][id],
    "callback function in Pulse.events[" .. event .. "][" .. id .. "]. This function may be unregistered in the Pulse")
  safety.ensureTable(self.cache[event],
    "self.cache[event] does not exist. This probably means Pulse:require() was called outside of Pulse:emit()'s callbacks. ")
