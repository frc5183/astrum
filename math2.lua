...
---@return number
function Vector2D:getMagnitude() return (((self.x ^ 2) + (self.y ^ 2)) ^ 0.5) end

--- Returns the angle of the Vector2D in radians
---@return number
function Vector2D:getAngle() return math.atan2(self.y, self.x) end
...
---@return Point2D
function Vector2D:toPoint() return Point2D.fromVector(self) end
...
---@return Vector2D
function Point2D:toVector() return Vector2D.fromPoint(self) end
...
function out.integrate(_math)
  for k, v in pairs(out) do if k ~= "integrate" then _math[k] = v end end
  return _math
end
